...
def read_mnist(root: Path, train: bool, limit: int | None = None) -> TensorDataset:
    ...
    Args:
        ...
        limit (int | None, optional): Maximum number of samples to load.
    ...
    images = np.frombuffer(image_bytes, dtype=np.uint8, offset=16, count=count * rows * cols).copy()
    ...
    labels = torch.from_numpy(
        np.frombuffer(label_bytes, dtype=np.uint8, offset=8, count=min(label_count, count)).copy()
    ).long()
...
calibration_loader = DataLoader(
    read_mnist(DATA_ROOT, train=True, limit=BATCH_SIZE),

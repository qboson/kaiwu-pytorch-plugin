...
def read_mnist(root: Path, train: bool) -> TensorDataset:
    ...
    labels = torch.from_numpy(np.frombuffer(data, dtype=np.uint8, offset=8, count=label_count).copy()).long()
...
quantized_model, report = quantization(model, bits=BITS, solvers=SOLVERS, layers=LAYERS).hessian(hessians)

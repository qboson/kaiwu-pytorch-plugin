...
from model import Config, MNISTQVAE, FeatureExtractor
...
class Trainer:
    ...
    def __init__(self, config, custom_train_data=None, custom_test_data=None):
        ...
        self.output_dir = (
            config.output_dir or f"./output/{self.config.type}_{self.timestamp}"
        )
    ...
    def _setup_tuner(self):
        ...
        # 创建优化器
        if self.config.type == "MNISTQVAE":
            vae_params = list(self.model.encoder.parameters()) + list(
                self.model.decoder.parameters()
            )
            vae_optimizer = torch.optim.Adam(vae_params, lr=self.config.lr)
            bm_optimizer = torch.optim.Adam(
                self.model.bm.parameters(), lr=self.config.bm_lr
            )
        ...
        # 设置训练偏置
        if self.config.type == "MNISTQVAE" and hasattr(self.model, "set_train_bias"):
    ...
    def train(
        ...
        epoch_pbar = tqdm(
            range(1, self.config.num_epochs + 1), desc="Training Progress"
        )
        checkpoint_interval = max(1, self.config.num_epochs // 10)
        for epoch in epoch_pbar:
            ...
            epoch_pbar.set_description(
                f"Epoch {epoch}/{self.config.num_epochs} - Train Loss: {train_loss:.2f}, Test Loss: {test_loss:.2f}"
            )
            ...
            if generate_animation and (
                epoch % tsne_interval == 0 or epoch == self.config.num_epochs
            ):
    ...
    def _save_energy_plot(self, energies, labels, output_path):
        ...
        median_order = (
            df.groupby("label")["energy"].median().sort_values().index.tolist()
        )
    ...
    def save_results(self):
        ...
        for k, v in self.config.__dict__.items():
            ...
            if k == "activation_fct":
                ...
                config_dict[k] = (
                    v.__class__.__name__ if hasattr(v, "__class__") else str(v)
                )
            else:
                config_dict[k] = v

...
class Encoder(nn.Module):
    ...
    def get_weight_decay(self) -> torch.Tensor:
        ...
        """
        return self.weight_decay * (torch.sum(self.fc1.weight ** 2) + torch.sum(self.fc2.weight ** 2))
...
class QVAE(nn.Module):
    ...
    def __init__(
        ...
        self.train_bias = torch.tensor(
            -np.log(1. / np.clip(mean_x, 0.001, 0.999) - 1.).astype(np.float32)
        )
        self.is_training = True

        # 网络
        self.encoder = Encoder(input_dim, hidden_dim, latent_dim, weight_decay=0.01)
        self.rbm = RestrictedBoltzmannMachine(num_visible=num_var1, num_hidden=num_var2,
                                              h_range=[-1, 1], j_range=[-1, 1])
    ...
    def cross_entropy(self, logit_q: torch.Tensor, log_ratio: torch.Tensor) -> torch.Tensor:
        ...
        # 计算交叉熵
        cross_entropy = -torch.matmul(torch.cat([q1, q2],dim=-1), self.rbm.linear_bias) +\
                -torch.sum(torch.matmul(q1_pert, self.rbm.quadratic_coef) * q2, dim=1, keepdim=True)

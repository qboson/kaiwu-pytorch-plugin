    ...
    def fit(self, X, Y_train, X_test, Y_test):
        ...
        for iteration in range(1, self.n_iter + 1):
            for batch_slice in batch_slices:
                ...
                if self.verbose:
                    if (idx - 1) % 5 == 0:
                        rf = linear_model.LogisticRegression(solver="newton-cholesky", tol=1e-5,
                                                             max_iter=1000, C=500)
                        predic.append(
                            rf.fit((rbm.get_hidden(X_torch)[:, rbm.num_visible:]), Y_train).score(
                                    (rbm.get_hidden(X_test_torch)[:, rbm.num_visible:]), Y_test))
                        print(f"Accuracy: {predic[-1]:.4f}")
                    if (idx - 1) % 20 == 0:                        
                        print("jmean", torch.abs(rbm.quadratic_coef).mean(),
                              "jmax", torch.abs(rbm.quadratic_coef).max())
                        print("hmean", torch.abs(rbm.linear_bias).mean(),
                              "hmax", torch.abs(rbm.linear_bias).max())
                        plt.figure(figsize=(16, 2))
                        display_samples = rbm.sample(self.sampler).cpu().numpy()[:20, :rbm.num_visible]

...
- `simple/`：最小训练和生成示例
- `qdiffusion_protein/`：基于 DPLM 的蛋白案例完整工作流与适配代码
...
- **DPLM 模型组装**：通过 `example/qdiffusion/qdiffusion_protein/utils/qdiffusion_protein_builder.py` 加载 proposal backbone 和 energy backbone，整理 token metadata，构建 energy adapter，并最终组装出一个通用 Q-Diffusion 实例
...
```bash
python example/qdiffusion/qdiffusion_protein/train_workflow.py
```
